...
def set_reminder(task: str, reminder_time: str, priority: str = "normal") -> str:
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "INSERT INTO reminders (task, deadline, next_notification, status, priority) VALUES (?, ?, ?, ?, ?)",
        ...
        return json.dumps(
            {
                "status": "success",
                "message": f"Reminder set successfully with {priority} priority.",
                "id": r_id,
            }
        )
...
def get_pending_reminders() -> str:
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT id, task, deadline, priority FROM reminders WHERE status = 'pending' ORDER BY datetime(deadline) ASC"
        )
        rows = c.fetchall()
        conn.close()
        if not rows:
...
def poll_due_reminders(skip_non_critical: bool = False) -> list:
    ...
    try:
        ...
        for row in rows:
            ...
            c.execute(
                "UPDATE reminders SET next_notification = ? WHERE id = ?",
                (next_time, row_dict["id"]),
            )
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "task": {
                        "type": "string",
                        "description": "The task or event to remember.",
                    },
                    "reminder_time": {
                        "type": "string",
                        "description": "ISO 8601 format WITH timezone offset (e.g. 2026-03-05T14:00:00+05:30) indicating when the reminder should fire.",
                    },
                    "priority": {
                        "type": "string",
                        "enum": ["low", "normal", "critical"],
                        "description": "Priority of the reminder. Low/Normal are suppressed during meetings and sleeping hours. Critical will interrupt meetings.",
                    },
                },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "reminder_id": {
                        "type": "integer",
                        "description": "The ID of the reminder to mark done.",
                    }
                },
