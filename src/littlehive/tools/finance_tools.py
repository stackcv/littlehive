...
def add_bill(vendor: str, amount: float, due_date: str, invoice_number: str = "Unknown") -> str:
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        ...
        conn.commit()
        conn.close()
        return json.dumps({"status": "success", "message": f"Bill recorded successfully.", "bill_id": bill_id})
...
def list_bills(status: str = "pending") -> str:
    try:
        ...
        if status.lower() == "all":
            ...
            c.execute('SELECT * FROM bills WHERE status = ? ORDER BY due_date ASC', (status.lower(),))
...
def mark_bill_paid(bill_id: int) -> str:
    try:
        ...
        conn.commit()
        conn.close()
        return json.dumps({"status": "success", "message": f"Bill #{bill_id} marked as paid."})
    except Exception as e:
        return json.dumps({"error": str(e)})
...
def delete_bill(bill_id: int) -> str:
    try:
        ...
        conn.commit()
        conn.close()
        return json.dumps({"status": "success", "message": f"Bill #{bill_id} deleted permanently."})
    except Exception as e:
        return json.dumps({"error": str(e)})
...
FINANCE_TOOLS_SCHEMA = [
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "vendor": {"type": "string", "description": "Name of the company or service."},
                    "amount": {"type": "number", "description": "The total amount due."},
                    "due_date": {"type": "string", "description": "The due date in YYYY-MM-DD format if known, otherwise best guess."},
                    "invoice_number": {"type": "string", "description": "The invoice or account number, if available."}
                },
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "status": {"type": "string", "enum": ["pending", "paid", "all"], "description": "Filter by status. Default is pending."}
                }
            }
        }
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "bill_id": {"type": "integer", "description": "The numeric ID of the bill to mark as paid."}
                },
                "required": ["bill_id"]
            }
        }
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "bill_id": {"type": "integer", "description": "The numeric ID of the bill to delete."}
                },
                "required": ["bill_id"]
            }
        }
