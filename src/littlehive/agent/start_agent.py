...
def inject_proactive_update(updates_found):
    ...
    if updates_found:
        ...
        inbox_queue.put({"source": "proactive", "text": system_prompt_injection, "chat_id": active_telegram_chat_id})
...
def is_user_busy():
    ...
    try:
        ...
        res_str = get_events(time_min=(now - timedelta(minutes=1)).isoformat(), time_max=(now + timedelta(minutes=1)).isoformat())
...
def check_reminders_job():
    ...
    for r in due_reminders:
        ...
        updates.append(f"⏰ REMINDER DUE: '{r['task']}'. Priority: {priority}. Internal ID {r['id']}. You must ask the user if they completed it.")
...
def check_apis_job():
        ...
        if "emails" in email_res:
            for email in email_res["emails"]:
                if email["id"] not in notified_email_ids:
                    ...
                    updates.append(f"📧 New Email (ID: {email['id']}): '{email['subject']}' from {email['sender']}")
    except Exception as e:
        pass
    ...
    try:
        ...
        event_res_str = get_events(time_min=now.isoformat(), time_max=next_hour.isoformat())
        ...
        if isinstance(event_res, list):
            for event in event_res:
                if event["id"] not in notified_event_ids:
                    ...
                    event_type = "Personal Block" if is_personal else f"Meeting with {len(attendees)} attendees"
                    updates.append(f"📅 {event_type}: '{event['summary']}' at {event['start']}")
    except Exception as e:
        pass
...
def nightly_db_cleanup():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("DELETE FROM system_logs WHERE datetime(timestamp) <= datetime('now', '-7 days')")
        cursor.execute("DELETE FROM pending_tasks WHERE status IN ('completed', 'failed') AND datetime(created_at) <= datetime('now', '-2 days')")
        cursor.execute("DELETE FROM reminders WHERE status = 'completed' AND datetime(deadline) <= datetime('now', '-2 days')")
        conn.commit()
        conn.close()
...
def trigger_nightly_memory():
    ...
    inbox_queue.put({"source": "system_maintenance", "text": "EXTRACT_NIGHTLY_MEMORIES", "chat_id": active_telegram_chat_id})


def process_pending_tasks_job():
    from littlehive.tools.task_queue import _get_db
    from littlehive.tools.email_tools import _actual_send_email, _actual_manage_email, _actual_reply_to_email
    from littlehive.tools.calendar_tools import _actual_create_event, _actual_update_event, _actual_delete_event
    ...
    try:
        ...
        for task in tasks:
            ...
            # Mark processing
            cursor.execute("UPDATE pending_tasks SET status='processing' WHERE id=?", (task_id,))
            conn.commit()
            
            func = executors.get(tool_name)
            if not func:
                cursor.execute("UPDATE pending_tasks SET status='failed', error_message='Unknown tool' WHERE id=?", (task_id,))
                conn.commit()
                continue
            ...
            try:
                ...
                cursor.execute("UPDATE pending_tasks SET status='completed', error_message=NULL WHERE id=?", (task_id,))
                conn.commit()
            except Exception as e:
                error_msg = str(e)
                new_retry = retry_count + 1
                if new_retry >= 3:
                    cursor.execute("UPDATE pending_tasks SET status='failed', error_message=? WHERE id=?", (error_msg, task_id))
                    conn.commit()
                    ...
                    inbox_queue.put({"source": "proactive", "text": injection, "chat_id": active_telegram_chat_id})
                else:
                    next_run = (datetime.now() + timedelta(minutes=2)).strftime("%Y-%m-%d %H:%M:%S")
                    cursor.execute("UPDATE pending_tasks SET status='failed_retry', retry_count=?, next_run_at=?, error_message=? WHERE id=?", (new_retry, next_run, error_msg, task_id))
                    conn.commit()
...
def start_proactive_scheduler():
    ...
    try:
        ...
        email_res = json.loads(email_res_str)
        if "emails" in email_res:
            for email in email_res["emails"]: notified_email_ids.add(email["id"])
        ...
        event_res = json.loads(event_res_str)
        if isinstance(event_res, list):
            for event in event_res: notified_event_ids.add(event["id"])
    except Exception:
        pass
    ...
    if config.get("poll_reminders_enabled", True):
        fast_secs = config.get("poll_reminders_interval", config.get("fast_polling_seconds", 30))
        scheduler.add_job(check_reminders_job, 'interval', seconds=fast_secs, next_run_time=datetime.now())
    ...
    if config.get("poll_tasks_enabled", True):
        ...
        scheduler.add_job(process_pending_tasks_job, 'interval', minutes=tasks_mins, next_run_time=datetime.now())
        
    # 3. External APIs (Google Calendar, Gmail lookahead)
    if config.get("poll_apis_enabled", True):
        api_minutes = config.get("poll_apis_interval", config.get("proactive_polling_minutes", 20))
        scheduler.add_job(check_apis_job, 'interval', minutes=api_minutes, next_run_time=datetime.now() + timedelta(seconds=10))
...
def telegram_worker():
    ...
    def edit_message(chat_id, message_id, text):
        if not text.strip(): return
        requests.post(f"{BASE_URL}/editMessageText", json={"chat_id": chat_id, "message_id": message_id, "text": text})
    ...
    def telegram_sender():
        ...
        while True:
            ...
            if msg.get("type") == "init":
                ...
                resp = requests.post(f"{BASE_URL}/sendMessage", json={"chat_id": active_chat_id, "text": "⚙️ Processing..."})
    ...
    while True:
        try:
            resp = requests.get(f"{BASE_URL}/getUpdates", params={"timeout": 30, "offset": update_offset}, timeout=35)
            updates = resp.json()
        except Exception:
            time.sleep(1)
            continue
        ...
        for update in updates["result"]:
            ...
            inbox_queue.put({"source": "telegram", "text": user_input, "chat_id": chat_id})
        ...
        try:
            if "[ARGS]" in block:
                ...
                try:
                    ...
                    json_calls = json.loads(block.replace('\n', '\\n').replace('\r', '\\r'))
            ...
            logger.warning(f"Warning: Failed to parse tool call block: {block[:100]}... Error: {str(e)}")
            continue
...
def main():
    config = get_config()
    model_path = config.get("model_path", "mlx-community/mistralai_Ministral-3-14B-Instruct-2512-MLX-MXFP4")
    
    logger.info(f"Initializing Core Brain & loading model ({model_path.split('/')[-1]})...")
    ...
    try:
        ...
        all_possible_tools = ROUTE_SCHEMAS["email"] # As per our Tiered Persona strategy, load all core tools
        ...
        chat_kwargs_warmup = {"tokenize": False, "add_generation_prompt": False, "tools": historically_active_tools}
        ...
        mx.eval(eval_list)
        logger.info(f"✅ [Brain] Cache warmed with {len(previous_prompt_tokens)} tokens! Agent is ready and fast.")
    ...
    while True:
        ...
        if source == "system_maintenance" and user_input == "EXTRACT_NIGHTLY_MEMORIES":
            try:
                ...
                conn = sqlite3.connect(DB_PATH)
                cursor = conn.cursor()
                cursor.execute("SELECT role, content FROM chat_archive WHERE datetime(timestamp) >= datetime('now', '-1 day')")
                chats = cursor.fetchall()
                conn.close()
                
                chat_text = "\n".join([f"{r[0]}: {r[1]}" for r in chats if r[1] and r[0] in ('user', 'assistant')])
                if chat_text.strip():
                    ...
                    temp_messages = [{"role": "system", "content": "You are a data extraction assistant. Output only a raw JSON list of strings."}, {"role": "user", "content": prompt}]
                    temp_prompt_str = tokenizer.apply_chat_template(temp_messages, tokenize=False, add_generation_prompt=True)
                    
                    # Run without using the main prompt_cache to keep conversation context pure
                    response = generate(model, tokenizer, prompt=temp_prompt_str, verbose=False, max_tokens=300)
                    try:
        ...
        class MultiOutbox:
            def put(self, msg):
                if source == "proactive" or source == "web":
                    outbox_web.put(msg)
                if source == "telegram" or (source == "proactive" and active_telegram_chat_id):
        ...
        if cmd in ["/reset", "/new"]:
            ...
            prompt_cache = make_prompt_cache(model)
            outbox.put({"type": "chunk", "content": "🧠 Memory wiped. Starting a fresh conversation."})
        ...
        try:
            while True: # Tool Chaining Loop
                ...
                full_prompt_text = tokenizer.apply_chat_template(messages, **chat_kwargs)
                ...
                for response in stream_generate(model, tokenizer, prompt=prompt_tokens, prompt_cache=prompt_cache, max_tokens=2048, sampler=sampler):
                ...
                if is_tool_call and not tool_calls_list:
                    ...
                    messages.append({"role": "assistant", "content": f"System Error: {err_msg}"})
                    break
                ...
                messages.append({"role": "assistant", "tool_calls": formatted_tool_calls, "content": ""})
                chat_kwargs["add_generation_prompt"] = False
            ...
            err_msg = str(e)
            outbox.put({"type": "error", "content": err_msg})
            messages.append({"role": "assistant", "content": f"Internal Error: {err_msg}"})
