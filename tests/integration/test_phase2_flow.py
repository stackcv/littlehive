...
class TestProvider(ProviderAdapter):
    ...
    def generate(self, request: ProviderRequest) -> ProviderResponse:
        return ProviderResponse(provider=self.name, model=request.model, output_text="phase2-reply", raw={"echo": request.prompt[:50]})
...
def phase2_runtime(tmp_path):
    ...
    executor = ToolExecutor(registry=registry, logger=get_logger("test.phase2"), call_logger=persist_tool_call)
    ...
    runtime = TelegramRuntime(
        ...
        logger=get_logger("test.phase2.runtime"),
    )
    return runtime, session_factory


@pytest.mark.asyncio
async def test_long_chat_generates_cards_and_bounded_context(phase2_runtime):
    runtime, session_factory = phase2_runtime
    for i in range(40):
...
@pytest.mark.asyncio
async def test_planner_transfer_execution_reply_flow(phase2_runtime):
    runtime, session_factory = phase2_runtime
...
@pytest.mark.asyncio
async def test_tool_schema_only_at_execution_step(phase2_runtime):
...
@pytest.mark.asyncio
async def test_telegram_runtime_phase2_smoke(phase2_runtime):
    runtime, _ = phase2_runtime
